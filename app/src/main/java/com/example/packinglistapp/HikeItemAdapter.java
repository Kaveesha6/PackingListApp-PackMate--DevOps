...
public class HikeItemAdapter extends RecyclerView.Adapter<HikeItemAdapter.ViewHolder> {

    private List<HikeItem> itemList;
    private Context context;

    public HikeItemAdapter(Context context, List<HikeItem> itemList) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        HikeItem item = itemList.get(position);
    ...
    public void addItem(HikeItem item) {
        itemList.add(item);
        notifyItemInserted(itemList.size() - 1);
    }

    public List<HikeItem> getItemList() {
        return itemList;
    }
