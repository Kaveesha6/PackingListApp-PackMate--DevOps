...
public class CampItemAdapter extends RecyclerView.Adapter<CampItemAdapter.ViewHolder> {
    ...
    private List<CampItem> itemList;
    private Context context;

    public CampItemAdapter(Context context, List<CampItem> itemList) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        CampItem item = itemList.get(position);
    ...
    public void addItem(CampItem item) {
        itemList.add(item);
        notifyItemInserted(itemList.size() - 1);
    }

    public List<CampItem> getItemList() {
        return itemList;
    }
