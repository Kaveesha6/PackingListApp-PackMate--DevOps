...
public class BeachItemAdapter extends RecyclerView.Adapter<BeachItemAdapter.ViewHolder> {

    private List<BeachItem> itemList;
    private Context context;

    public BeachItemAdapter(Context context, List<BeachItem> itemList) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        BeachItem item = itemList.get(position);
    ...
    public void addItem(BeachItem item) {
        itemList.add(item);
        notifyItemInserted(itemList.size() - 1);
    }

    public List<BeachItem> getItemList() {
        return itemList;
    }
