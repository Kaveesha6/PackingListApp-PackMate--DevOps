...
public class SwimItemAdapter extends RecyclerView.Adapter<SwimItemAdapter.ViewHolder> {

    private List<SwimItem> itemList;
    private Context context;

    public SwimItemAdapter(Context context, List<SwimItem> itemList) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        SwimItem item = itemList.get(position);
    ...
    public void addItem(SwimItem item) {
        itemList.add(item);
        notifyItemInserted(itemList.size() - 1);
    }

    public List<SwimItem> getItemList() {
        return itemList;
    }
